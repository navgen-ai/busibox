...
class TestAuditWithBearerContext:
    """Tests for audit logging with JWT bearer token context."""
    
    @pytest.mark.asyncio
    async def test_audit_records_user_context_from_token(self, admin_headers, db_pool, clean_test_data):
        """Test that audit endpoint uses caller context from bearer token."""
        import jwt
        import json
        ...
        async with db_pool.acquire() as conn:
            ...
            await conn.execute(
                """
                INSERT INTO authz_roles (id, name)
                VALUES ($1, $2)
                """,
                uuid.UUID(role_id),
        ...
        async with httpx.AsyncClient() as client:
            ...
            async with db_pool.acquire() as conn:
                row = await conn.fetchrow(
                    ...
                    WHERE actor_id = $1 AND action = 'doc.move'
