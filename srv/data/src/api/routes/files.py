...
@router.post("/reprocess-all", dependencies=[Depends(require_data_write)])
    ...
    try:
        async with postgres_service.acquire(request) as conn:
            ...
            query = """
                ...
                FROM data_files
                WHERE data_status IN ('completed', 'failed')
            """
            params = []
...
@router.get("/{fileId}", dependencies=[Depends(require_data_read)])
...
@router.get("/{fileId}/download", dependencies=[Depends(require_data_read)])
...
@router.get("/{fileId}/presigned-url", dependencies=[Depends(require_data_read)])
...
@router.get("/{fileId}/chunks", dependencies=[Depends(require_data_read)])
...
@router.get("/{fileId}/vectors", dependencies=[Depends(require_data_read)])
    ...
    try:
        async with postgres_service.acquire(request) as conn:
            ...
            vector_count_row = await conn.fetchrow("""
                SELECT vector_count FROM data_files WHERE file_id = $1
            """, uuid.UUID(fileId))
...
@router.post("/{fileId}/search", dependencies=[Depends(require_data_read)])
...
@router.delete("/{fileId}", dependencies=[Depends(require_data_delete)])
...
@router.post("/{fileId}/move", dependencies=[Depends(require_data_write)])
...
@router.post("/{fileId}/reprocess", dependencies=[Depends(require_data_write)])
        ...
        async with postgres_service.acquire(request) as conn:
            ...
            if start_stage in stages_needing_chunk_delete:
                deleted_chunks = await conn.execute("""
                    DELETE FROM data_chunks WHERE file_id = $1
                """, uuid.UUID(fileId))
