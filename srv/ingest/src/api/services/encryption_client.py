...
Zero Trust Authentication:
    All methods require a user_token parameter - either:
    - The user's JWT from the incoming request (for user-initiated operations)
    - A delegation token (for service-to-service operations like background processing)
    
    The token is passed to authz which validates it and checks the user's
    role access to the file being encrypted/decrypted.

Usage:
    client = EncryptionClient(config)
    
    # On upload (pass user's token from request)
    ...
    # On download (pass user's token from request)
...
class EncryptionClient:
    ...
    Uses Zero Trust authentication - all operations require a user token
    passed from the request context. No admin tokens or client credentials.
    """
    ...
    def __init__(self, config: dict):
        ...
        self.enabled = ENCRYPTION_ENABLED and bool(self.authz_base_url)
        ...
        if not self.enabled:
            ...
            logger.info(
                "Envelope encryption client initialized (Zero Trust mode)",
                authz_base_url=self.authz_base_url,
            )
    
    def _get_headers(self, user_token: str) -> dict:
        """Get headers for keystore API calls.
        
        Args:
            user_token: The user's JWT token from the request context
        """
        return {
    ...
    async def ensure_kek_for_role(self, role_id: str, user_token: str) -> Optional[dict]:
        """
        Ensure a KEK exists for a role.
        
        Args:
            role_id: Role UUID
            user_token: User's JWT token from request context
    ...
    async def encrypt_for_upload(
        ...
        Args:
            ...
            user_token: User's JWT token from request context (required)
        ...
        if not user_token:
            logger.error("No user_token provided for encrypt_for_upload", file_id=file_id)
            return content
    ...
    async def add_role_access(self, file_id: str, role_id: str, user_token: str) -> bool:
        """
        ...
        if not user_token:
            logger.error("No user_token provided for add_role_access", file_id=file_id)
            return False
    ...
    async def remove_role_access(self, file_id: str, role_id: str, user_token: str) -> bool:
        """
    ...
    async def delete_file_keys(self, file_id: str, user_token: str) -> bool:
        """
