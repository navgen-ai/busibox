...
class DataService:
    ...
    Data documents are stored in the ingestion_files table with doc_type='data'.
            ...
            if cached:
                ...
                async with self.acquire_with_rls(request) as conn:
                    exists = await conn.fetchval(
                        "SELECT 1 FROM ingestion_files WHERE file_id = $1 AND doc_type = 'data'",
                        uuid.UUID(document_id)
                    )
                    if exists:
        ...
        async with self.acquire_with_rls(request) as conn:
            ...
            if expected_version is not None:
                current_version = await conn.fetchval(
                    "SELECT data_version FROM ingestion_files WHERE file_id = $1 AND doc_type = 'data'",
                    uuid.UUID(document_id)
                )
    ...
    async def delete_document(
        ...
        async with self.acquire_with_rls(request) as conn:
            result = await conn.execute(
                "DELETE FROM ingestion_files WHERE file_id = $1 AND doc_type = 'data'",
                uuid.UUID(document_id)
            )
