...
Zero Trust Authentication:
    All decryption operations require a user_token parameter - either:
    - The user's JWT from the incoming request (for user-initiated operations)
    - A delegation token (for background processing / worker operations)
    
    The token is passed to authz which validates it and checks the user's
    role access to the file being decrypted.
"""
...
class FileService:
    ...
    Uses Zero Trust authentication - decryption operations require a user token
    ...
    def __init__(self, config: dict):
        ...
        # Encryption settings (Zero Trust - tokens come per-request, not stored)
        self.authz_base_url = config.get("authz_base_url") or os.getenv(
            "AUTHZ_BASE_URL", "http://10.96.201.210:8010"
        )
        encryption_enabled = os.getenv("ENCRYPTION_ENABLED", "true").lower() == "true"
        self.encryption_enabled = encryption_enabled and bool(self.authz_base_url)
        
        if self.encryption_enabled:
            logger.info(
                "FileService encryption enabled (Zero Trust mode)",
    ...
    def _decrypt_content_sync(
        ...
        if not user_token:
            logger.error("No user_token provided for decryption", file_id=file_id)
            return encrypted_content
    ...
    def download(
        ...
        Args:
            ...
            user_token: User's JWT token from request context (required for decryption)
        ...
        try:
            ...
            if file_id and user_token and is_encrypted(content):
                logger.info(
            ...
            elif file_id and is_encrypted(content) and not user_token:
                logger.warning(
                    "Content appears encrypted but no user_token provided - skipping decryption",
                    file_id=file_id,
                    content_size=len(content),
                )
