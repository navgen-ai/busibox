    ...
    while true; do
        ...
        case $choice in
            ...
            7)
                ...
                ;;
            8)
                if confirm "Deploy vLLM (Main) to $env?"; then
                    deploy_service "vllm" "$env"
                fi
                pause
                ;;
            9)
                if confirm "Deploy vLLM Embedding to $env?"; then
                    deploy_service "vllm-embedding" "$env"
                fi
                pause
                ;;
            10)
                if confirm "Deploy ColPali to $env?"; then
