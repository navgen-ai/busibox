...
#
# USAGE:
#   From ansible dir: bash scripts/bootstrap-test-credentials.sh [docker|staging|production] [--force|-f]
#   From repo root:   bash provision/ansible/scripts/bootstrap-test-credentials.sh [docker|staging|production] [--force|-f]
#
# OPTIONS:
    ...
    if [ -n "$ADMIN_TOKEN_FOR_CLIENT" ] && [ -n "$JWT_SECRET" ]; then
        ...
        CREATE_RESPONSE=$(curl -s -X POST "${AUTHZ_URL}/admin/oauth-clients" \
            ...
            -d "{
                ...
                \"allowed_audiences\": [\"data-api\", \"search-api\", \"agent-api\", \"authz\"],
