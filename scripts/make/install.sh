...
create_env_file() {
    ...
    # NOTE: This file contains both configuration and secrets.
    # Secrets are generated by generate_secrets() and must be present in this
    # file so docker-compose.yml can inject them into containers. The same
    # secrets are also synced to the Ansible vault for Proxmox deployments.
...
#
# This file is read by docker-compose.yml via --env-file.
# It contains both configuration and secrets needed by containers.
# The same secrets are also stored in the encrypted Ansible vault.
...
EOF
    ...
    chmod 600 "$env_file"
    success "Created ${env_file} (with generated secrets)"
}
