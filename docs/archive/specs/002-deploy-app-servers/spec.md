...
1. **Given** NGINX is configured with SSL certificates, **When** a user navigates to agents.ai.localhost, **Then** they are served the agent-manager app over HTTPS with a valid certificate
2. **Given** subdomain routing is configured, **When** a user accesses docintel.ai.localhost, **Then** they are routed to the docintel application
3. **Given** sub-directory routing is configured, **When** a user accesses ai.localhost/agents, **Then** they are routed to the agent-manager application
...
1. **Given** a user navigates to ai.localhost (naked domain), **When** the page loads, **Then** they see the main portal landing page
2. **Given** a user navigates to www.ai.localhost or ai.localhost/home, **When** the page loads, **Then** they see the same main portal landing page
...
1. **Given** the agent-manager is deployed, **When** an administrator navigates to agents.ai.localhost, **Then** they can access the agent administration interface
2. **Given** an administrator navigates to ai.localhost/agents, **When** the page loads, **Then** they can access the same agent administration interface
...
- **FR-013**: NGINX MUST serve all traffic over HTTPS with valid SSL certificates for the ai.localhost domain and subdomains
...
- **FR-014**: System MUST support subdomain-based virtual hosts (e.g., agents.ai.localhost, docintel.ai.localhost)
- **FR-015**: System MUST support sub-directory routing (e.g., ai.localhost/agents, ai.localhost/docintel)
...
- **FR-019**: System MUST deploy the main portal application from github.com/jazzmind/busibox to the apps-lxc container
- **FR-020**: Main portal MUST be accessible at the naked domain (ai.localhost), www subdomain (www.ai.localhost), and /home path (ai.localhost/home)
...
- **FR-026**: Agent-client MUST be accessible at agents.ai.localhost and ai.localhost/agents
...
- **SSL Certificate**: TLS/SSL certificate for the ai.localhost domain and wildcards for subdomains; includes private key and certificate chain
...
- The main portal (busibox) implements standard session-based authentication that can be shared across applications via cookies or headers
...
- DNS records for ai.localhost and *.ai.localhost point to the NGINX proxy IP address
