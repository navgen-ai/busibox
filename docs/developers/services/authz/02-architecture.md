...
category: "developer"
order: 51
description: "OAuth2-aligned RBAC and token exchange implementation for Busibox"
published: true
---
...
- Same configuration as data-api
...
2. **Update test services with authz config**:
   ```bash
   make ingest INV=inventory/staging
   make search INV=inventory/staging
   make agent INV=inventory/staging
   make apps INV=inventory/staging
   ```
...
curl -X POST http://10.96.200.210:8010/oauth/token \
  ...
  -d '{"grant_type":"client_credentials","client_id":"ai-portal","client_secret":"<secret>","audience":"data-api"}'
