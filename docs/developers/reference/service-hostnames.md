...
category: "developer"
order: 141
description: "Canonical hostnames for Busibox services across Docker and Proxmox"
published: true
---
...
| Data API | `data-api` | 8002 | `data` | Document ingestion & data storage |
...
// Fetch from API
const response = await fetch("http://data-api:8002/files");
```
...
docker exec local-data-api ping -c 1 postgres
docker exec local-data-api getent hosts authz-api
docker exec local-data-api curl http://milvus:9091/healthz
```
...
- `docs/archive/developers-old/architecture/internal-dns-routing.md` - Architecture overview (archived)
