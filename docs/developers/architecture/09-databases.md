...
- [Database Commands](../reference/database-commands.md) — psql, migrations, backups
...
|----------|-------|---------|
| `test_agent` | `busibox_test_user` | Agent service pytest |
| `test_authz` | `busibox_test_user` | AuthZ service pytest |
| `test_data` | `busibox_test_user` | Data service pytest |
...
- name: Run database migrations
  ...
  environment:
    DATABASE_URL: "postgresql+asyncpg://busibox_user:{{ password }}@{{ host }}/agent"

- name: Run test database migrations
  command: alembic upgrade head
  environment:
    DATABASE_URL: "postgresql+asyncpg://busibox_test_user:{{ password }}@{{ host }}/test_agent"
...
See [Database Separation Migration Guide](../reference/database-separation-migration.md) if it exists; otherwise run from repo root as above.
...
```bash
# Agent API
DATABASE_URL=postgresql+asyncpg://busibox_user:${PASSWORD}@${POSTGRES_HOST}:5432/agent
...
```bash
# Agent API (pytest)
TEST_DATABASE_URL=postgresql+asyncpg://busibox_test_user:${PASSWORD}@${HOST}:5432/test_agent
...
pg_dump -h ${HOST} -U postgres agent > agent_$(date +%Y%m%d).sql
pg_dump -h ${HOST} -U postgres authz > authz_$(date +%Y%m%d).sql
pg_dump -h ${HOST} -U postgres data > data_$(date +%Y%m%d).sql

# Restore
psql -h ${HOST} -U postgres -d agent < agent_backup.sql
```
...
```bash
psql -h ${HOST} -U postgres -c "\l" | grep agent
```
