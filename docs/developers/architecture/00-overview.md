...
**Related Docs**:  
- [Service Hostnames](../reference/service-hostnames.md) — IPs and ports by environment  
...
- **Databases**: PostgreSQL in `pg-lxc` hosts per-service databases:
  - `agent` - Agent definitions, conversations, workflows
  - `authz` - Users, roles, sessions, audit logs
  - `data` - File metadata, ingestion status, chunks
  - `ai_portal` - Portal-specific data (Prisma managed)
  - Plus test databases (`test_agent`, `test_authz`, `test_data`) for pytest isolation
