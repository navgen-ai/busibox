...
**Parameters:**
- `SERVICE`: authz, data, search, agent, all
...
| `INV` | staging | Environment: staging or production |
| `FAST` | 1 | Skip `@pytest.mark.slow` and `@pytest.mark.gpu` tests |
| `WORKER` | 0 | Start local data worker for full pipeline tests |
...
# Combine options
make test-local SERVICE=data INV=staging FAST=0 ARGS="-k encryption --tb=long"
...
make test-local SERVICE=data WORKER=1 FAST=0

# Run specific pipeline tests with worker
make test-local SERVICE=data WORKER=1 ARGS="tests/integration/test_full_pipeline.py"
```
...
```bash
ssh root@10.96.201.206  # data container
cd /srv/data
...
# Direct - DO NOT USE UNLESS ABSOLUTELY NECESSARY
ssh root@10.96.201.206 "cd /srv/data && source venv/bin/activate && pytest tests/ -v"
```
...
make test SERVICE=data INV=staging  # Check output for patterns
```
...
```
→ See [Python Test Import Gotchas](../reference/python-test-import-gotchas.md)
...
ssh root@10.96.201.206 "systemctl status data-api data-worker"
...
```bash
# Get service logs
ssh root@10.96.201.206 "journalctl -u data-api -n 100 --no-pager"
ssh root@10.96.201.206 "journalctl -u data-worker -n 100 --no-pager"
```
...
ssh root@10.96.201.206 "cat /srv/data/.env | grep -v PASSWORD"

# Verify PYTHONPATH
ssh root@10.96.201.206 "cd /srv/data && source venv/bin/activate && python -c 'import sys; print(sys.path[:5])'"
```
...
find /srv/data/src -name "redis.py" -o -name "minio.py"
...
```bash
# Clear pycache
find /srv/data -type d -name '__pycache__' -exec rm -rf {} + 2>/dev/null
```
...
|---------|---------------|-------|---------------------|
| Agent | `test_agent` | `busibox_test_user` | `agent` |
| AuthZ | `test_authz` | `busibox_test_user` | `authz` |
| Data | `test_data` | `busibox_test_user` | `data` |
...
| data       | 10.96.201.206   | 8002  |
| search     | 10.96.201.204   | 8003  |
...
- [Python Test Import Gotchas](../reference/python-test-import-gotchas.md) - Module import debugging
- [Bootstrap Test Credentials](../services/authz/04-testing.md) - Setting up test auth
