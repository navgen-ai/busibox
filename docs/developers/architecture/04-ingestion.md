...
- `POST /files/{fileId}/search` -- per-file hybrid search (semantic + BM25). Cross-document search is in the [Search API](../services/search/03-api.md).
...
**Detailed docs**: [services/data/](../services/data/01-overview.md)

## Auth & RLS
- JWT middleware (`Authorization: Bearer`) validates RS256 tokens from AuthZ service.
- Validates audience claim is `data-api` and issuer is `busibox-authz`.
