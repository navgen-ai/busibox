...
### 2. Run Tests via Makefile Targets
Use the new extraction test targets from admin workstation:
- `make test-extraction-simple` - SIMPLE strategy (baseline)
- `make test-extraction-llm` - SIMPLE + LLM cleanup
- `make test-extraction-marker` - Marker strategy (server only)
- `make test-extraction-colpali` - ColPali visual extraction (GPU required)
