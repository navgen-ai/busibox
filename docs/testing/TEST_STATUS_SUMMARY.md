...
```bash
# Test from admin workstation using Makefile targets
cd /Users/wessonnenreich/Code/sonnenreich/busibox/provision/ansible

# Run SIMPLE extraction tests (baseline - should all pass)
make test-extraction-simple INV=inventory/test

# Run SIMPLE + LLM cleanup tests
make test-extraction-llm INV=inventory/test

# Run Marker extraction tests (server only - resource intensive)
make test-extraction-marker INV=inventory/test

# Run ColPali visual extraction tests (requires GPU service)
make test-extraction-colpali INV=inventory/test

# Or run directly on ingest-lxc container
ssh root@10.96.200.206  # test environment
cd /srv/ingest && source venv/bin/activate
python -m pytest tests/test_pdf_extraction_simple.py -v
python -m pytest tests/test_pdf_extraction_llm.py -v
python -m pytest tests/test_pdf_extraction_marker.py -v
python -m pytest tests/test_colpali_extraction.py -v
```
